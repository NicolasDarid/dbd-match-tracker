...
`;
    ...
    log(
      "\n⚠️  IMPORTANT: Vous devez configurer votre DATABASE_URL dans le fichier .env",
      "yellow"
    );
    log(
      "📖 Consultez DATABASE_SETUP.md pour les options de base de données",
      "cyan"
    );
    ...
    if (continueSetup.toLowerCase() !== "y") {
      log(
        "Configuration interrompue. Configurez votre DATABASE_URL et relancez le script.",
        "yellow"
      );
      process.exit(0);
    }
...
async function main() {
  log(
    "🚀 Configuration de Prisma et de la base de données pour DBD Match Tracker",
    "bright"
  );
  log("=".repeat(70), "cyan");
  ...
  try {
    ...
    if (!prerequisitesOk) {
      log(
        "\n❌ Prérequis non satisfaits. Veuillez les installer et relancer le script.",
        "red"
      );
      process.exit(1);
    }
    ...
    log(
      "🚀 Vous pouvez maintenant lancer votre application avec: pnpm dev",
      "cyan"
    );
  } catch (error) {
